...
- **(b) Alea seeding prevents mirroring.** Each `MapNoiseConfig.seed` is fed through `alea` to produce a distinct PRNG before `createNoise2D`. Using a single seed for both axes would correlate elevation and moisture and produce a visible diagonal mirror artefact.
